  ...
  navbarPage( "Stroke Companion",  # tabsetPanel(
           ...
           tabPanel("Blood Pressure",
                    ...
                    fluidRow( 
                      ...
                      #        # label = h3("Weeks Range")
                      # )
                      column(3, radioButtons("radioBP", label = h3("BP"),
                                             choices = list("Systolic" ="sys" , "Diastolic" ="dia", "Heart Rate" ="hr"  ), 
                                             selected = "sys")
                             ),
                      column(3 , radioButtons("radioTimeframe", label = h3("Timeframe"),
                                             choices = list("Week" = "week" , "Month" = "month", "Other" = "other" ), 
                                             selected = "week") )
                    ),
                    plotOutput("plotBPHR")
...
server <- function(input, output) {
  ...
  output$plotBPHR <- renderPlot({ dashboard(yvar=input$radioBP,period = input$radioTimeframe) }  )
